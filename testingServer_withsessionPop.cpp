...
void server::broadcast(){
    ...
    while(true){
        sessionLock.lock();
        vectorSize = currentConnection.size();
        if (vectorSize == 0){
            continue;
        }
        std::vector<std::vector<session*>::iterator> bePopped;
        for(auto sessionPtr = currentConnection.begin(); sessionPtr != currentConnection.end(); ++sessionPtr){
            if ((*sessionPtr)->fail){
                bePopped.push_back(sessionPtr);
            }
        }
        for(auto iter: bePopped){
            currentConnection.erase(iter);
        }
        vectorSize = currentConnection.size();
        sessionLock.unlock();
